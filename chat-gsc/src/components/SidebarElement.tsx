...
const SidebarElement = ({ text, href }: { text: string;  href: string}) => {
    ...
    return (
        <li>
            <Link href={href} className={`flex h-16 md:h-11 items-center p-2 text-gray-100 rounded-lg dark:text-white hover:bg-teal-800 ${path == href ? "bg-teal-800" : null} dark:hover:bg-gray-700 group`} onMouseEnter={() => setIsHovering(true)} onMouseOut={() => setIsHovering(false)}>
