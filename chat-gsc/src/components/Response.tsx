...
/* NEED TO DO:
- Move the scroll logic to the parent component
    - pass down ref to response
- add event listener to see if scrollTop decreases
- if it does, set shouldScroll to false for 7 seconds
 */
...
const Response = ({ question, questionFinished, children, questionPosted, setQuestionPosted }:
    ...
    // useEffect(() => {
    //     const handleScroll = ({event} : {event: Event}) => {
    //         console.log("scrollTop: " + dummyRef.current?.scrollTop);
    //         console.log("scrollHeight: " + dummyRef.current?.scrollHeight);
    //         console.log("clientHeight: " + dummyRef.current?.clientHeight);
    //     }

    //     if (dummyRef.current) {
    //         dummyRef.current.addEventListener('scroll', handleScroll);
    //         if (Math.abs(dummyRef.current.scrollHeight - dummyRef.current.scrollTop - dummyRef.current.clientHeight) < 20) {
    ...
    //     return () => {
    //         if (dummyRef.current) {
    //             dummyRef.current.removeEventListener('scroll', handleScroll);
    //         }
    //     };
    // }, []);

    // const childrenWithProps = React.Children.map(children, child => {
    //     // Checking if it's a valid element
    //     if (React.isValidElement(child)) {
    //         return React.cloneElement(child, { shouldScroll });
    //     }
    //     return child;
    // });
    ...
    return (
        <div ref={dummyRef} className="overflow-y-scroll text-neutral-600">
            <div className={`w-full border-b flex flex-row ${questionFinished ? null : "invisible"}`}>
                ...
                <div className="mr-24 mt-10 mb-10 flex items-center text-sm">
                   {question}
                </div>
            </div>
